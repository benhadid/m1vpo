---
type: assignment
date: 2022-05-19T10:00:00+1:00
title: 'Reconstruction 3D'
attachment: /static_files/assignments/hw3.zip
weight: '40%'
hide_from_announcements: True
due_event:
    type: due
    date: 2022-06-05T17:00:00+1:00
    description: 'Devoir #4 - à remettre'
---

# Démarche à suivre  

  1. Depuis votre machine VM, téléchargez le fichier de démarrage (fichier ```hw3.zip```) du devoir et décompressez le dans le répertoire de votre choix. 

  2. Depuis un terminal, accédez au répertoire du devoir (par exemple hw3) :
      ```bash
      user@vlab$ cd hw3
      ```      

  3. Lancez le bloc-notes jupyter à l'aide de la commande :   
      ```bash
      user@vlab$ jupyter notebook
      ```      

  4. Un navigateur Web devrait démarrer en affichant la liste des fichiers dans le répertoire courant. Cliquez sur le fichier associé au devoir (fichier avec l’extension `.ipynb`) pour afficher le bloc-notes.

  5. Lisez attentivement les questions et les commentaires dans le bloc-notes. Pour évaluer une réponse à une question du bloc-notes, exécutez la cellule (Cell \| Run Cells) associée à la question.

  6. Si vous rencontrez des problèmes avec votre VM (machine devenant très lente ou ne répond plus aux commandes), augmentez la taille mémoire allouée à votre VM depuis la console de VirtualBox.